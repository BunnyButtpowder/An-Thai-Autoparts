...
// motif, carrying the final contact call to action for the commercial services page.
export default function CommercialServicesCTA() {
  const sectionRef = useReveal<HTMLElement>((g, root) => {
    g.set('.commercial-services-cta-reveal', { y: 32, opacity: 0 })
    g.timeline({ scrollTrigger: { trigger: root, start: 'top 80%', once: true } })
      .to('.commercial-services-cta-reveal', { duration: 0.7, y: 0, opacity: 1, stagger: 0.12, ease: 'power3.out' })
  })
  ...
  return (
    <section
      ref={sectionRef}
      id="lien-he"
      className="commercial-services-cta-section relative isolate overflow-hidden bg-primary py-24"
      aria-labelledby="commercial-services-cta-heading"
    >
      <div
        className="commercial-services-cta-grid pointer-events-none absolute inset-0 opacity-[0.1]"
        style={{
          backgroundImage:
            'linear-gradient(to right, rgba(255,255,255,0.7) 1px, transparent 1px), linear-gradient(to bottom, rgba(255,255,255,0.4) 1px, transparent 1px)',
