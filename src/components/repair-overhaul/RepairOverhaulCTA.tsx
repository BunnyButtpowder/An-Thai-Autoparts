...
// motif, carrying the final contact call to action for the repair centre page.
export default function RepairOverhaulCTA() {
  const sectionRef = useReveal<HTMLElement>((g, root) => {
    g.set('.repair-overhaul-cta-reveal', { y: 32, opacity: 0 })
    g.timeline({ scrollTrigger: { trigger: root, start: 'top 80%', once: true } })
      .to('.repair-overhaul-cta-reveal', { duration: 0.7, y: 0, opacity: 1, stagger: 0.12, ease: 'power3.out' })
  })
  ...
  return (
    <section
      ref={sectionRef}
      id="lien-he"
      className="repair-overhaul-cta-section relative isolate overflow-hidden bg-primary py-24"
      aria-labelledby="repair-overhaul-cta-heading"
    >
      <div
        className="repair-overhaul-cta-grid pointer-events-none absolute inset-0 opacity-[0.1]"
        style={{
          backgroundImage:
