    ...
    >
      <div className="about-news-container relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="about-news-header flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="about-news-eyebrow-label inline-flex items-center px-3 py-1 rounded-full bg-accent text-primary text-lg font-semibold uppercase tracking-wider">
              Cập nhật
            </p>
            <h2
              id="about-news-heading"
              className="about-news-heading mt-4 text-4xl font-extrabold uppercase leading-tight tracking-tight text-foreground sm:text-5xl"
            >
              Tin tức &amp; Báo chí
            </h2>
          </div>
          <Link
            to="/tin-tuc"
            className="about-news-view-all group inline-flex shrink-0 items-center gap-2 pb-1.5 text-base font-semibold uppercase tracking-widest text-primary transition-colors hover:text-primary-hover cursor-pointer"
          >
            Tất cả bài viết
            ...
            <article key={article.id} className="about-news-card flex">
              ...
              >
                <div className="about-news-card-media relative aspect-16/10 w-full shrink-0 overflow-hidden bg-foreground">
                  ...
                  />
                  <span className="about-news-card-tag absolute left-4 top-4 rounded-md bg-primary px-3 py-1.5 text-sm font-bold uppercase tracking-widest text-primary-foreground">
                    {article.categoryLabel}
                  </span>
                </div>
                <div className="about-news-card-body flex flex-1 flex-col p-6 sm:p-7">
                  <span className="about-news-card-date text-base font-semibold tabular-nums tracking-[0.02em] text-muted-foreground">
                    {article.postedDate}
                  </span>
                  <h3 className="about-news-card-title mt-3 text-xl font-bold leading-snug text-foreground transition-colors group-hover:text-primary">
                    {article.title}
                  </h3>
                  <p className="about-news-card-excerpt mt-3 line-clamp-2 text-base leading-relaxed text-muted-foreground">
                    {article.excerpt}
                  </p>
                  <span className="about-news-card-cta mt-auto inline-flex items-center gap-1.5 pt-6 text-base font-semibold uppercase tracking-widest text-primary">
                    Đọc tiếp
                    <ArrowRight className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" />
                  </span>
                </div>
